 ...
 ********************************************************************/
	...
	$reputasi = $_REQUEST["reputasi"];
	$tiperepu = $_REQUEST["tiperepu"];
	$desrepu = $_REQUEST["desrepu"];
	$user = $_SESSION["Username"];
	$today = date("Y-m-d H:i:s");

	$cekreputasi = mysql_query("SELECT * FROM reputasi WHERE ReputasiBy = '".$user."' AND UserID = '".$reputasi."' AND DATE_FORMAT(InputDate, '%Y-%m-%d') = DATE_FORMAT(NOW(),'%Y-%m-%d')");
	$num_row = mysql_num_rows($cekreputasi);
	...
	$querytoshare = mysql_query("SELECT * FROM usermst WHERE (UserID = '".$reputasi."' OR UserGroup ='".$reputasi."') AND UserEmail <> ''");
	$li_row = 1;
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            ...
                                                            <td>
                                                                ...
                                                                <div style="color:#676767;font-family:Arial;font-size:14px;line-height:18px;text-align:left">'.$fnamefrom.' ('.$emailfrom.') .</div>
                                                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                                    <tbody>
                                                                        <tr>
                                                                ...
                                                                <div style="color:#676767;font-family:Arial;font-size:14px;line-height:18px;text-align:left">Memberikan reputasi '.$tiperepu.' kepada anda.</a></div>
                                                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                                    <tbody>
                                                                        <tr>
                                                                            <td style="font-size:0pt;line-height:0pt;text-align:left" height="20"></td>
	...
	$ls_subject = "[App E-Office] ".$user." add reputation with you - ".$desrepu;
	...
	if($num_row>0){
		$pesan = "Anda sudah memberikan reputasi hari ini, silahkan berikan kembali besok.";
	}else{
		$pesan = "Sudah diberikan";
		mysql_query("INSERT INTO reputasi (UserID,idcat,idtop, ReputasiBy,TipeReputasi,DeskripsiReputasi,InputDate)
		VALUES('".$reputasi."','".$catid."','".$topicid."','".$user."','".$tiperepu."','".$desrepu."','".$today."')");
	...
	header("location:viewpost.php?idcat=".$catid."&top=".$topicid."&pesan=".$pesan);
?>