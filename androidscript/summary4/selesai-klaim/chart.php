...
$querySelect1 = "SELECT  HariGroup1, HariGroup2, HariGroup3, HariGroup4, HariGroup5, nmproduk
				FROM    (
						SELECT  nmproduk,
						...
						FROM    (
							...
							) AS hari, pr.regional, p.nmproduk 
							...
							AND cn.confirm_claim = 'Approve(paid)'
								) AS SubQueryAlias GROUP BY nmproduk
						) AS SubQueryAlias2
				GROUP BY nmproduk";
				...
				if ($checkrows > 0){
					while ($getData = mysql_fetch_assoc($result1))
					{
						...
						$data1['data'][] = array(
											...
											'nm_produk'	=> is_null($getData['nmproduk']) ? '0' : $getData['nmproduk']
											);
					}
				}else{
					$data1 = array();
		...
		$querySelect = "SELECT  HariGroup1, HariGroup2, HariGroup3, HariGroup4, HariGroup5, nmproduk
				FROM    (
						SELECT  nmproduk,
						COUNT(CASE WHEN  hari < 14 THEN hari < 14 END) AS HariGroup1,
								COUNT(CASE WHEN  hari >= 14 AND hari <= 28 THEN hari >= 14 AND hari <= 28 END) AS HariGroup2,
