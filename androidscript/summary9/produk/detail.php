...
$queryUserID 	= "SELECT id,branch,wilayah,idbroker, idclient, regional, level FROM useraccess WHERE id='$userID'";
...
$userArea 	= $encodedUserID['wilayah'];
...
if ($usersType == "Direksi" AND $userlevel == "14")
{
    if($nmgrup == 'All Report'){
...
elseif($usersType == "Kadiv" AND $userlevel =="13"){
	if($nmgrup == 'All Report'){
	    ...
	    {
	        $filterquery = "AND ajkdebitnote.idbroker = '$idbroker' AND ajkdebitnote.idclient = '$idclient' AND ajkpeserta.regional='$userRegional' AND ajkpeserta.area = '$userArea'";
	    }
	    $filterquery2 = "GROUP BY ajkpeserta.cabang,ajkdebitnote.idproduk";
	    $filterselect = "ajkcabang.name AS regional";
	}else{
		$filterquery = "AND ajkdebitnote.idbroker = '$idbroker' AND ajkdebitnote.idclient = '$idclient' AND ajkpeserta.regional='$userRegional' AND ajkpeserta.area = '$userArea' AND ajkpolis.produk = '$nmgrup'";
		$filterquery2 = "GROUP BY ajkpeserta.cabang,ajkdebitnote.idproduk";
		$filterselect = "ajkcabang.name AS regional";
	}

}elseif($usersType == "Kadiv" AND $userlevel =="12"){
    $queryCabang 	= "select `name`,level from ajkcabang where er = '$userCabang'";
    $resultCabang 	= query_db($queryCabang);
    $encodedCabang 	=  mysql_fetch_assoc($resultCabang);
    $cabangLevel 	= $encodedCabang['level'];


    if($nmgrup == 'All Report'){
        if ($cabangLevel == '1')
        {
            $filterquery = "AND ajkdebitnote.idbroker = '$idbroker' AND ajkdebitnote.idclient = '$idclient' AND ajkpeserta.area = '$userArea'";
            $filterquery2 = "GROUP BY ajkpeserta.cabang,ajkdebitnote.idproduk";
            $filterselect = "ajkcabang.name AS regional";
        }
        elseif($cabangLevel == '2')
        {
            $filterquery = "AND ajkdebitnote.idbroker = '$idbroker' AND ajkdebitnote.idclient = '$idclient' AND ajkpeserta.area = '$userArea' AND ajkcabang.`level` IN ('2','3')";
            $filterquery2 = "GROUP BY ajkpeserta.cabang,ajkdebitnote.idproduk";
            $filterselect = "ajkcabang.name AS regional";
        }
    }
