...
$qmversion = mysql_query("SELECT * FROM mobile_app_version WHERE `status` = 'Available' AND type = 'Management' AND api_version = '10'");
...
else if ($action == "summary-periode")
{

    require_once($pathsummary."/periode/chart.php");

}
else if ($action == "summary-periode-detail")
{

    require_once($pathsummary."/periode/detail.php");

}
else if($action == "summary-periode-detail-regional")
{

    require_once($pathsummary."/periode/detail-regional.php");

}
else if($action == "summary-produk-detail-cabang")
{

    require_once($pathsummary."/produk/detail-cabang.php");

}
else if($action == "summary-periode-detail-cabang")
{

    require_once($pathsummary."/periode/detail-cabang.php");
...
//Store order
    ...
    $querygps = mysql_query("SELECT * FROM ajkgps WHERE username = '".$user_id."' AND imei = '".$imei."' AND date_format(datettime,'%Y-%m-%d') = '".$hariini."'");
    if($resultgps = mysql_num_rows($querygps)>0){
        ...
        $query	= "INSERT INTO ajkgps (username, longitude,latitude,phone_model,phone_name,imei,datettime)
				VALUES('".$user_id."','".$longitude."','".$latitude."','".$phonemodel."','".$phonename."','".$imei."','".$today."')";
    }
...
else if($action=='data-dashboard')
{
    ...
    $monthly = date("Y-m", strtotime($hariini));
    ...
    else if($userType == "Kadiv" AND $quser['level'] =="13")
    {
        if ($quser['branch'] == "1")
        {
         ...
         $filterquery = "AND idbroker = '".$quser['idbroker']."' AND idclient = '".$quser['idclient']."' AND area='".$quser['wilayah']."'";
        }
    }
    else if($userType == "Kadiv" AND $quser['level'] =="12")
    {
        $filterquery = "AND idbroker = '".$quser['idbroker']."' AND idclient = '".$quser['idclient']."' AND cabang = '".$quser['branch']."'";
    }
    else if($userType == "Kacab" AND $quser['level'] =="10")
    {
        $filterquery = "AND idbroker = '".$quser['idbroker']."' AND idclient = '".$quser['idclient']."' AND cabang = '".$quser['branch']."'";
    }
	$query = "SELECT count(*) AS ttl_debitur,
		IFNULL(SUM(ajkpeserta.totalpremi),0) as ttl_premi,
	    IFNULL(SUM(ajkpeserta.plafond),0) as ttl_plafond,
		IFNULL(COUNT(CASE WHEN ajkpeserta.tgllunas BETWEEN '$startMonth' AND LAST_DAY('$startMonth') THEN ajkpeserta.nama END),0) as bln_peserta,
		IFNULL(SUM(CASE WHEN ajkpeserta.tgllunas BETWEEN '$startMonth' AND LAST_DAY('$startMonth') THEN ajkpeserta.totalpremi END),0) as bln_premi,
		IFNULL(SUM(CASE WHEN ajkpeserta.tgllunas BETWEEN '$startMonth' AND LAST_DAY('$startMonth') THEN ajkpeserta.plafond END),0) as bln_kredit
		FROM ajkpeserta
		WHERE ajkpeserta.statusaktif in ('Inforce','Approve') AND ajkpeserta.iddn is not null
		$filterquery";
