...
$qmversion = mysql_query("SELECT * FROM mobile_app_version WHERE `status` = 'Available' AND type = 'Management' AND api_version = '5'");
...
else if ($action == "search-list-group")
{
	...
	$querySelect = "SELECT ajkpolis.produk as nmproduk FROM ajkpolis
									WHERE ajkpolis.idcost = '$search' and general = 'T'";
...
else if ($action == "summary-produk-detail")
{
	...
	require_once("summary3/produk/detail.php");
...
else if($action=='data-dashboard')
{
    ...
    if($idbroker != "0"){
        $filterbroker = "AND idbroker = '".$idbroker."'";
    }
    if($idclient != "0"){
        $filterclient = "AND idclient = '".$idclient."'";
    }
    ...
    else if ($userType == "Kadiv" AND $userlevel == "13")
    {
         $filterType = "AND ajkpeserta.regional='$userRegional'";
    }
    else if ($userType == "Kadiv" AND $userlevel == "12")
    {
         $filterType = "AND ajkpeserta.cabang='$userCabang'";
    }
	...
	$query = "SELECT count(*) AS ttl_debitur,
		...
		$filterclient $filterbroker $filterType";
...
else if ($action == 'ubah-password')
{
    ...
    $resultUserID 	= query_db($queryUserID);
    $encodedUserID 	=  mysql_fetch_assoc($resultUserID);
    $passw 	= $encodedUserID['passw'];
    $mamet 	= $encodedUserID['mamet'];
    $encodedUserID 	= $encodedUserID['id'];

    $today = date('Y-m-d H:i:s');

    if ($old_pass_enc != $passw)
    {
        $json['err_no'] = '2';
        $json['err_msg'] = 'Password lama tidak sama dengan yang sebelumnya';
        echo json_encode($json);
    }
    else
    {
        $queryUpdate =  "UPDATE useraccess SET passw = '$new_pass_enc', mamet = '$new_pass', update_by = '$userID', update_time = '$today' WHERE id = '$userID'";
        $resultUpdate = query_db($queryUpdate);
