...
$qmversion = mysql_query("SELECT * FROM mobile_app_version WHERE `status` = 'Available' AND type = 'Marketing' and api_version = '5'");
...
else if($action=='data-dashboard')
{
	...
	if($userType=="Admin"){
		...
		FROM ajkspk WHERE del IS NULL ";

		$result = query_db($query);
// $sessionlogout AS sessionlogout $filterusr
		if($result)
		{
			if(mysql_num_rows($result)>0)
			{
