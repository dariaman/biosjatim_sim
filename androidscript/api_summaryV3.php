...
$qmversion = mysql_query("SELECT * FROM mobile_app_version WHERE `status` = 'Available' AND type = 'Management' AND api_version = '3'");
...
else if($action=='data-dashboard')
{
	...
	if($deviceVersion <> $versionavailable)
	{
		...
		if($quser['idbroker'] != "0"){
			$filterbroker = "AND idbroker = '".$quser['idbroker']."'";
		}
		if($quser['idclient'] != "0"){
			$filterclient = "AND idclient = '".$quser['idclient']."'";
		}
